...
public class TGoogleConnect extends BaseConnect {
    ...
    public TGoogleConnect(Context context) {
        super(context);
    ...
    public void connectToGetAddress(final double latitude, final double longitude) {
        ...
        if (isNetWorkable()) {
            ...
            String url = MessageFormat.format(ConnectInfo.T_API_GOOGLE_GEOCODE, latitude + "," + longitude, "zh-TW");
    ...
    public void connectToGetDirection(String startLat, String startLng, String endLat, String endLng) {
        ...
        if (isNetWorkable()) {
            String url = MessageFormat.format(ConnectInfo.T_API_GOOGLE_DIRECTION, origin, destination, mode, key, "zh-TW");
            request = new Request.Builder().url(url).get().build();
            client.newCall(request).enqueue(new Callback() {
                @Override
                public void onFailure(Call call, IOException e) {
                ...
                @Override
                public void onResponse(Call call, Response response) throws IOException {
                    directionInfo = gson.fromJson(response.body().string(), DirectionInfo.class);
    ...
    public void connectToGetPlace(String lat, String lng, String type) {
        ...
        if (isNetWorkable()) {
            loadingImageCircleDialog.showLoadingDialog();
            String url = MessageFormat.format(ConnectInfo.T_API_GOOGLE_PLACE, location, radius, type, language, key);
            Log.e("TTTTTT", url);
...
//    }
    ...
    public void connectToGetDistance(double startLat, double startLng, double endLat, double endLng) {
        ...
        if (isNetWorkable()) {
            String url = MessageFormat.format(ConnectInfo.T_API_GOOGLE_MATRIX, origins, destinations, language, key);
            request = new Request.Builder().url(url).get().build();
            client.newCall(request).enqueue(new Callback() {
                @Override
                public void onFailure(Call call, IOException e) {
                ...
                @Override
                public void onResponse(Call call, Response response) throws IOException {
                    distanceInfo = gson.fromJson(response.body().string(), DistanceInfo.class);
